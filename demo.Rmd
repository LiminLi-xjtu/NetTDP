...
print(paste('lower (1-alpha)-confidence bound for the number of true discoveries in the set is',res.abs.node,'(abs.node)'))
######## edge-level inference
G<-G.abs.edge
nEdge <- dim(G)[2]
k <- ceiling(nEdge * 200 * myalpha)
thr <- sort(as.vector(G), decreasing=TRUE)[k]   # threshold: k-th greatest statistic
res.abs.edge <- tdp(sumStats(G, alpha = mylpha, truncFrom = thr, truncTo = 0))
print(paste('lower (1-alpha)-confidence bound for the number of true discoveries in the set is',res.abs.edge,'(sign.abs.edge)'))
...
for (i in 1:ncluster) {
  ...
  res.abs <- sumStats(G,mysubset,alpha = 0.05)
  tdp.abs.node[i] <- tdp(res.abs) 
}

G<-G.abs.edge
adj <- matrix(0,ngene,ngene)
tdp.abs.edge <-  c()
for (i in 1:ncluster) {
  print(paste('i =', i))
  mysubset <- which(moduleColors==cluster[i])
  ...
  res.abs <- sumStats(G, S = mysubset.edge, alpha = alpha,truncFrom = thr, truncTo = 0)
